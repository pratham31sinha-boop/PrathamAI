...
def _stream_groq(messages):
    ...
    yield from _stream_openai_compatible("[https://api.groq.com/openai/v1/chat/completions](https://api.groq.com/openai/v1/chat/completions)", GROQ_API_KEY, "llama-3.3-70b-versatile", messages)
...
def _stream_openrouter(messages):
    ...
    yield from _stream_openai_compatible("[https://openrouter.ai/api/v1/chat/completions](https://openrouter.ai/api/v1/chat/completions)", OPENROUTER_API_KEY, "meta-llama/llama-3.3-70b-instruct", messages)
...
def _stream_cerebras(messages):
    ...
    yield from _stream_openai_compatible("[https://api.cerebras.ai/v1/chat/completions](https://api.cerebras.ai/v1/chat/completions)", CEREBRAS_API_KEY, "llama3.3-70b", messages)

def _stream_mistral(messages):
    if not MISTRAL_API_KEY or _is_cooling("mistral"): raise RuntimeError("Mistral matrix unavailable.")
    yield from _stream_openai_compatible("[https://api.mistral.ai/v1/chat/completions](https://api.mistral.ai/v1/chat/completions)", MISTRAL_API_KEY, "mistral-large-latest", messages)
...
def fetch_github_repository_tree_matrix():
    ...
    endpoint_target_url = f"[https://api.github.com/repos/](https://api.github.com/repos/){repo_clean}/git/trees/main?recursive=1"
...
def execute_python():
    ...
    if not code: return jsonify({"error": "Empty tracking runtime input buffer context"}), 400
